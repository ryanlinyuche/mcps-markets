...
// ── Bubble-aware bet placement ─────────────────────────────────────────────

export async function placeBubbleBet(
  userId: number,
  marketId: number,
  bubbleId: number,
  side: 'YES' | 'NO',
  ...
  const txn = await db.transaction('write')
  try {
    const memberRes = await txn.execute({
      sql: 'SELECT balance FROM bubble_members WHERE bubble_id = ? AND user_id = ?',
      args: [bubbleId, userId],
    })
    const member = memberRes.rows[0] as unknown as { balance: number } | undefined
    if (!member) throw new Error('You are not a member of this bubble')
    if (member.balance < amount) throw new Error('Insufficient bubble balance')

    const marketRes = await txn.execute({
      sql: 'SELECT status, bubble_id FROM markets WHERE id = ?',
      args: [marketId],
    })
    const market = marketRes.rows[0] as unknown as { status: string; bubble_id: number | null } | undefined
    if (!market) throw new Error('Market not found')
    if (market.status !== 'open') throw new Error('Market is not open for betting')
    if (market.bubble_id !== bubbleId) throw new Error('Market does not belong to this bubble')

    await txn.execute({
      sql: 'UPDATE bubble_members SET balance = balance - ? WHERE bubble_id = ? AND user_id = ?',
      args: [amount, bubbleId, userId],
    })
    const poolCol = side === 'YES' ? 'yes_pool' : 'no_pool'
    await txn.execute({ sql: `UPDATE markets SET ${poolCol} = ${poolCol} + ? WHERE id = ?`, args: [amount, marketId] })
    ...
    await txn.execute({
      sql: `INSERT INTO transactions (user_id, type, amount, market_id, bubble_id, description) VALUES (?, 'bet_placed', ?, ?, ?, ?)`,
      args: [userId, -amount, marketId, bubbleId, `Bet ${amount} bubble coins on ${side}`],
    })
    ...
    const balRes = await txn.execute({
      sql: 'SELECT balance FROM bubble_members WHERE bubble_id = ? AND user_id = ?',
      args: [bubbleId, userId],
    })
    const newBal = balRes.rows[0] as unknown as { balance: number }
...
export async function placeBubbleScoreBet(
  userId: number,
  marketId: number,
  bubbleId: number,
  optionLabel: string,
  ...
  try {
    ...
    const marketRes = await txn.execute({
      sql: 'SELECT status, market_type, bubble_id FROM markets WHERE id = ?',
      args: [marketId],
    })
    const market = marketRes.rows[0] as unknown as { status: string; market_type: string; bubble_id: number | null } | undefined
    if (!market) throw new Error('Market not found')
    if (market.status !== 'open') throw new Error('Market is not open for betting')
    if (market.bubble_id !== bubbleId) throw new Error('Market does not belong to this bubble')

    const optRes = await txn.execute({
      sql: 'SELECT label FROM option_pools WHERE market_id = ? AND label = ?',
      args: [marketId, optionLabel],
    })
    if (!optRes.rows[0]) throw new Error('Invalid option')
    ...
    await txn.execute({
      sql: 'UPDATE option_pools SET amount = amount + ? WHERE market_id = ? AND label = ?',
      args: [amount, marketId, optionLabel],
    })
    ...
    await txn.execute({
      sql: `INSERT INTO transactions (user_id, type, amount, market_id, bubble_id, description) VALUES (?, 'bet_placed', ?, ?, ?, ?)`,
      args: [userId, -amount, marketId, bubbleId, `Bet ${amount} bubble coins on "${optionLabel}"`],
    })
...
// ── Bubble-aware resolution ────────────────────────────────────────────────

export async function resolveBubbleMarket(
  marketId: number,
  bubbleId: number,
  outcome: 'YES' | 'NO',
  resolvedBy?: number,
  ...
  try {
    const marketRes = await txn.execute({
      sql: 'SELECT status, yes_pool, no_pool FROM markets WHERE id = ? AND bubble_id = ?',
      args: [marketId, bubbleId],
    })
    const market = marketRes.rows[0] as unknown as { status: string; yes_pool: number; no_pool: number } | undefined
    if (!market) throw new Error('Market not found in this bubble')
    ...
    if (winPool === 0) {
      ...
      for (const pos of allRes.rows as unknown as { user_id: number; coins_bet: number }[]) {
        ...
        await txn.execute({
          sql: `INSERT INTO transactions (user_id, type, amount, market_id, bubble_id, description) VALUES (?, 'refund', ?, ?, ?, 'Market resolved with no winning bets')`,
          args: [pos.user_id, pos.coins_bet, marketId, bubbleId],
        })
      }
    } else {
      const winnersRes = await txn.execute({
      ...
      for (const pos of winnersRes.rows as unknown as { user_id: number; coins_bet: number }[]) {
        ...
        await txn.execute({
          sql: 'UPDATE bubble_members SET balance = balance + ? WHERE bubble_id = ? AND user_id = ?',
          args: [payout, bubbleId, pos.user_id],
        })
        await txn.execute({
          sql: `INSERT INTO transactions (user_id, type, amount, market_id, bubble_id, description) VALUES (?, 'payout', ?, ?, ?, ?)`,
          args: [pos.user_id, payout, marketId, bubbleId, `Won ${payout} bubble coins - resolved ${outcome}`],
        })
      }
    }
...
export async function resolveBubbleScoreMarket(
  marketId: number,
  bubbleId: number,
  winningLabel: string,
  resolvedBy?: number,
  ...
  try {
    const marketRes = await txn.execute({
      sql: 'SELECT status FROM markets WHERE id = ? AND bubble_id = ?',
      args: [marketId, bubbleId],
    })
    const market = marketRes.rows[0] as unknown as { status: string } | undefined
    if (!market) throw new Error('Market not found in this bubble')
    ...
    if (winPool === 0) {
      ...
      for (const pos of winnersRes.rows as unknown as { user_id: number; coins_bet: number }[]) {
        ...
        await txn.execute({
          sql: `INSERT INTO transactions (user_id, type, amount, market_id, bubble_id, description) VALUES (?, 'payout', ?, ?, ?, ?)`,
          args: [pos.user_id, payout, marketId, bubbleId, `Won ${payout} bubble coins - resolved "${winningLabel}"`],
        })
      }
    }
...
export async function resolveBubbleMarketNA(
  marketId: number,
  bubbleId: number,
  resolvedBy: number,
  notes: string | null,
  ...
  try {
    ...
    for (const pos of allRes.rows as unknown as { user_id: number; coins_bet: number }[]) {
      ...
      await txn.execute({
        sql: `INSERT INTO transactions (user_id, type, amount, market_id, bubble_id, description) VALUES (?, 'refund', ?, ?, ?, 'Market cancelled (N/A) - refunded')`,
        args: [pos.user_id, pos.coins_bet, marketId, bubbleId],
      })
    }
