...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json([], { status: 200 }) // return empty list on error so UI doesn't break
  }
}
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Check user not banned (column may not exist on older DBs — treat missing as not banned)
    try {
    ...
    } catch { /* column doesn't exist yet — skip ban check */ }
    ...
    try {
      ...
      if (market.comments_restricted && !session.isAdmin) {
        return NextResponse.json({ error: 'Comments are restricted on this market' }, { status: 403 })
      }
    } catch { /* column doesn't exist yet — allow comments */ }
